package o;

import android.opengl.Matrix;
import java.util.Locale;

/* renamed from: o.td1  reason: case insensitive filesystem */
/* loaded from: classes.dex */
public final class C9779td1 {
    public static final float[] a = new float[16];

    public static void a(float[] fArr, float f, float f2) {
        Matrix.translateM(fArr, 0, -f, -f2, 0.0f);
    }

    public static void b(float[] fArr, float f, float f2) {
        Matrix.translateM(fArr, 0, f, f2, 0.0f);
    }

    public static void c(float[] fArr, float f, float f2, float f3) {
        float[] fArr2 = a;
        synchronized (fArr2) {
            try {
                Matrix.setIdentityM(fArr2, 0);
                b(fArr2, f2, f3);
                Matrix.rotateM(fArr2, 0, f, 0.0f, 0.0f, 1.0f);
                a(fArr2, f2, f3);
                try {
                    Matrix.multiplyMM(fArr, 0, fArr2, 0, fArr, 0);
                } catch (Throwable th) {
                    th = th;
                    fArr2 = fArr2;
                    Throwable th2 = th;
                    throw th2;
                }
            } catch (Throwable th3) {
                th = th3;
            }
        }
    }

    public static void d(float[] fArr, float f, float f2, float f3) {
        b(fArr, f2, f3);
        Matrix.rotateM(fArr, 0, f, 0.0f, 0.0f, 1.0f);
        a(fArr, f2, f3);
    }

    public static void e(float[] fArr, float f) {
        b(fArr, 0.0f, f);
        Matrix.scaleM(fArr, 0, 1.0f, -1.0f, 1.0f);
        a(fArr, 0.0f, f);
    }

    public static void f(float[] fArr, float f, float f2, float f3) {
        Matrix.setIdentityM(fArr, 0);
        d(fArr, f, f2, f3);
    }

    public static String g(float[] fArr, int i) {
        return String.format(Locale.US, "Matrix:\n%2.1f %2.1f %2.1f %2.1f\n%2.1f %2.1f %2.1f %2.1f\n%2.1f %2.1f %2.1f %2.1f\n%2.1f %2.1f %2.1f %2.1f", Float.valueOf(fArr[i]), Float.valueOf(fArr[i + 4]), Float.valueOf(fArr[i + 8]), Float.valueOf(fArr[i + 12]), Float.valueOf(fArr[i + 1]), Float.valueOf(fArr[i + 5]), Float.valueOf(fArr[i + 9]), Float.valueOf(fArr[i + 13]), Float.valueOf(fArr[i + 2]), Float.valueOf(fArr[i + 6]), Float.valueOf(fArr[i + 10]), Float.valueOf(fArr[i + 14]), Float.valueOf(fArr[i + 3]), Float.valueOf(fArr[i + 7]), Float.valueOf(fArr[i + 11]), Float.valueOf(fArr[i + 15]));
    }
}
